...
public class PhysicCheck : MonoBehaviour
{
    ...
    public void 检查()
    {
        ...
        isGround = Physics2D.OverlapCircle(
            (Vector2)transform.position + 检测偏移量,
            检测范围,
            检测图层
        );
        #region 撞左墙判断
        是否撞左墙 = Physics2D.OverlapCircle(
            (Vector2)transform.position + 左墙偏移量,
            检测范围,
            检测图层
        );
        #endregion
        #region 撞右墙判断
        是否撞右墙 = Physics2D.OverlapCircle(
            (Vector2)transform.position + 右墙偏移量,
            检测范围,
            检测图层
        );
        #endregion
